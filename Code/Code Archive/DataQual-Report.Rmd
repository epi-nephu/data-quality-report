...
outbreak.raw <- readxl::read_xlsx(file.path(phess_data_dir, outbreak_file)) %>% janitor::clean_names()
```
...
key_missing_rmk <- cond_data1mth %>% 
  ...
  mutate(rmk_sympt = if_else(is.na(sympt_onset) & condition=="Mycobacterium ulcerans", "Symptom onset date is missing", NA_character_), 
