...
```{r define files}
condition_file <- "NEPHUCaseLinelist.xlsx"

outbreak_file <- "NEPHUOutbreakLinelist.xlsx"

wd <- getwd()
main_dir <- str_subset(wd, "^C:./Epi reports")
...
```{r load data}
condition.raw <- readxl::read_xlsx(paste0(phess_data_dir, condition_file), sheet=1, guess_max=min(500000, Inf)) %>% janitor::clean_names() 

# condition.raw <- readxl::read_xlsx(here::here("Data", condition_file), sheet=1, guess_max=min(500000, Inf)) %>% janitor::clean_names() 

outbreak.raw <- readxl::read_xlsx(paste0(phess_data_dir, outbreak_file)) %>% janitor::clean_names()
```
...
condition.clean <- condition.raw %>% 
  ...
  rename(defn = most_recent_event_classfication, 
         ...
         sexual_exposure = sexual_exposure_205, 
  ...
  filter(follow_up_required_by=="Local Public Health Unit") %>% 
...
risk_exclu_conditions <- c("Respiratory Syncytial virus", "Varicella zoster infection (Chickenpox)", "Varicella zoster infection (Shingles)", 
                           "Varicella zoster infection (Unspecified)", "Influenza")
