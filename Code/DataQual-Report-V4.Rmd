---
title: "NEPHU - Data quality check report (V4)"
author: "EPI-NEPHU"
...
- updated Shigella DQ check using criteria from DH DQ dashboard (treatment antibiotics check)
- updated AMR DQ check using criteria from DH DQ Dashboard
- removed condition=influenza for RSV outbreaks
- import separate case linelist files into Exceptions and Others
- remove TRA outbreaks from DQ check since this is DH responsibility

Version 4:
include export of Excel file containing only variables needed to transfer to the HP Log sheet - this entails recoding the functions used to create and output the tables

- tidied up AMR (cases up to 4 months for checking core data fields, added logic condition to filter out data checks on os related acquisition only on cases who reported travelling overseas in last 4yrs)
- split Other Conditions into AMR and Other (Legionella)
- filter DQ check on STI/BBV for ESF_collected = "Yes". HIV and Mpox - incl all. 
- amended syphilis symptom onset date check only for symptoms=Yes
- exclude 6 months follow-up treatment outcome data check for MU, see email from DH 27/11/2025 (note: have not exclude in-progress check yet, kept in to monitor for any not closed off.)
- removed aggregate tables section
- rabies and lyssavirus to remain as in progress - no need to check for Complete (ref: Renae email Epi inbox 9/1/2026)
- incl incident classification check for ROB (ref: Hibaq email and DH email Epi inbox 13/4/2026)
- check for risk factor for past 2 years only for HepC newly acquired cases (ref: Amina email Epi inbox 22/6/2026)
...
here::i_am("Code/DataQual-Report-V4.Rmd")
...
```{r define files}
condition_allothers_file <- "NEPHUCaseLinelistDQReportAllOthers.xlsx"

condition_exceptions_file <- "NEPHUCaseLinelistDQReportExceptions.xlsx"
...
```{r load data}
allothers.raw <- readxl::read_xlsx(file.path(phess_data_dir, condition_allothers_file), sheet=1, guess_max=min(500000, Inf)) %>% janitor::clean_names() 

exceptions.raw <- readxl::read_xlsx(file.path(phess_data_dir, condition_exceptions_file), sheet=1, guess_max=min(500000, Inf)) %>% janitor::clean_names() 

condition.raw <- bind_rows(allothers.raw, exceptions.raw)
...
enteric_vacc <- c("Typhoid", "Hepatitis A")
...
outbreak.clean <- outbreak.raw %>% 
  ...
  filter(! str_detect(event_name, "^[:digit:]{6}\\sTRA\\s")) %>% 
  filter(lphu=="North Eastern") 
...
cond_data_mth4 <- filter(condition.clean, event_date <= as.Date(start_date4mth)) %>% 
  filter(event_type=="Case") # incl cases where event data is before the 4mth cutoff
...
in_progress_mth2 <- cond_data_mth2 %>% 
  ...
  filter(!condition %in% c("Rabies", "Lyssavirus - Australian Bat Lyssavirus")) %>% 
  mutate(url = paste0('https://phess.dhhs.vic.gov.au/main.do?CaseID=', phess_id)) %>% 
...
output_keymiss_table <- function(df) {
  table = df
...
set_keymiss_ob_table <- function(df) {
  ...
  table <- df %>% 
    ...
    mutate(date_identified = Sys.Date()) %>% 
    filter(remarks!="NA") %>% 
    select(phess_id, url, condition, event_date, defn, investigation_status, remarks, date_identified, disease_type, event_name)
  
  return(table)
}
 
output_keymiss_ob_table <- function(df) {
  table = df
...
output_inprogress_table <- function(group, df) {
  table = df
  
  if(nrow(table)==0 & group %in% c("Blood Borne Viruses", "Other Conditions")) {
    cat("Data quality check complete - no cases marked as in progress after 3 months")
...
bbv_key_missing_rmk <- key_missing_rmk %>% 
  distinct(phess_id, .keep_all = TRUE) %>% 
  filter(disease_type=="Blood Borne Viruses") %>% 
  filter(esf_collected=="Yes") %>% 
  # combine data for HIV (all cases)
  bind_rows(., key_missing_rmk %>% distinct(phess_id, .keep_all=TRUE) %>% filter(condition %in% hiv_cond)) %>% 
  mutate(
    ...
    rmk_risk2yr = if_else(condition %in% c("Hepatitis B - Newly acquired", "Hepatitis C - Newly acquired") & 
...
bbv_keymiss_table <- set_keymiss_table(bbv_key_missing_rmk)

output_keymiss_table(bbv_keymiss_table)
...
bbv_inprogress_table <- set_inprogress_table("Blood Borne Viruses", 4)
output_inprogress_table("Blood Borne Viruses", bbv_inprogress_table)
...
ent_key_missing_nonshig <- key_missing_rmk %>% 
  ...
  mutate(
    ...
    rmk_vacc_typ = if_else(condition %in% enteric_vacc & is.na(vaccinated), 
                           "Vaccinated field is missing", NA_character_)
  )
...
shigella_key_missing <- key_missing_rmk %>% 
  ...
  group_by(phess_id) %>% 
  # remove duplicate rows by selecting the top 1 row in each group
  slice_head(n = 1) %>% 
  mutate(
    ...
    rmk_antib = if_else(treated=="Yes" & is.na(treated_antibiotic), "Antibiotic treated with missing", NA_character_), 
...
ent_keymiss_table <- set_keymiss_table(ent_key_missing_rmk)
output_keymiss_table(ent_keymiss_table)
...
crypto_risk_table <- cond_data1mth %>% 
  distinct(phess_id, .keep_all = TRUE) %>% 
  ...
  mutate(url = paste0('https://phess.dhhs.vic.gov.au/main.do?CaseID=', phess_id), 
         ...
         disease_type = "Enteric Diseases")

if(nrow(crypto_risk_table)==0) {
    cat("Data quality check complete - No crypto cases where exposure site is required is missing")
  } else if(nrow(crypto_risk_table)>0) {
flextable(crypto_risk_table, col_keys=c("phess_id", "condition", "event_date", "defn", "investigation_status", "remarks")) %>% 
  compose(j="phess_id", value=as_paragraph(hyperlink_text(x=phess_id, url=url))) %>% 
  color(j="phess_id", color="#0077CC") %>% 
...
ent_inprogress_table <- set_inprogress_table("Enteric Diseases")
output_inprogress_table("Enteric Diseases", ent_inprogress_table)
...
sti_key_missing_rmk <- key_missing_rmk %>% 
  ...
  mutate(
    ...
    rmk_symptonset = if_else(condition%in%syph_cond & symptoms=="Yes" & is.na(sympt_onset), 
...
sti_keymiss_table <- set_keymiss_table(sti_key_missing_rmk)
output_keymiss_table(sti_keymiss_table)
...
mpox_keymiss_table <- set_keymiss_table(mpox_key_missing_rmk)
output_keymiss_table(mpox_keymiss_table)
...
sti_inprogress_table <- set_inprogress_table("Sexually Transmissible Infections")
output_inprogress_table("Sexually Transmissible Infections", sti_inprogress_table)
...
vpd_keymiss_table <- set_keymiss_table(vpd_key_missing_rmk)
output_keymiss_table(vpd_keymiss_table)
...
vpd_inprogress_table <- set_inprogress_table("Vaccine Preventable Diseases")
output_inprogress_table("Vaccine Preventable Diseases", vpd_inprogress_table)
...
vbd_key_missing_rmk <- key_missing_rmk %>% 
  ...
  mutate(
    ...
    #                        "Treatment outcomes form requested missing", NA_character_), 
...
vbd_keymiss_table <- set_keymiss_table(vbd_key_missing_rmk)
output_keymiss_table(vbd_keymiss_table)

```
<br>

**Cases still marked as in progress after 1 month, after 3 months for RRV and BFV**
...
#   mutate(url = paste0('https://phess.dhhs.vic.gov.au/main.do?CaseID=', phess_id)) %>% 
#   select(phess_id, url, condition, event_date, defn, investigation_status, disease_type, event_type) %>% 
#   arrange(condition, event_date) %>% 
#   distinct(phess_id, .keep_all=TRUE) %>% 
#   mutate(date_identified = Sys.Date(), 
...
vbd_inprogress_table <- bind_rows(vbd_inprogress_2m, vbd_inprogress_4m)

  
if(nrow(vbd_inprogress_table)==0) {
  cat("Data quality check complete - no cases marked as in progress after 1 month or 3 months for RRV and BFV")
...
zoo_keymiss_table <- set_keymiss_table(zoo_key_missing_rmk)
output_keymiss_table(zoo_keymiss_table)
...
zoo_inprogress_table <- set_inprogress_table("Zoonotic")
output_inprogress_table("Zoonotic", zoo_inprogress_table)

```
<br><br>


### Other Conditions - AMR

**Cases marked completed with key data missing**

``` {r OTH key table, ft.align="left"}
# Other conditions excl AMR
oth_keymiss_data <- condition.clean %>% 
  # select data up to 4 months
  filter(event_date >= as.Date(start_date4mth)) %>% 
...
oth_key_missing_rmk <- oth_keymiss_data %>% 
  mutate(
    # core data fields
    rmk_suburb = if_else(is.na(suburb), "Suburb is missing", NA_character_), 
    rmk_pc = if_else(is.na(postcode), "Postcode is missing", NA_character_), 
...
# AMR
amr_key_missing_rmk <- oth_key_missing_rmk %>%  
  filter(condition %in% amr_cond) %>% 
  # filter out cases lost to follow-up
  filter(esf_requested!="Lost to follow-up") %>% 
  mutate(
    rmk_esf = if_else(condition %in% amr_cond & esf_requested!="Lost to follow-up" & is.na(esf_collected), 
                      "Outstanding ESF", NA_character_), 
    ...
    rmk_cty_reason_oth = if_else(condition %in% amr_cond & cpeb_travel_os_in_past_4y=="Yes" & cpeb_travel_os_in_past_4y_reason=="Other" & 
    ...
    rmk_med_intended_type = if_else(condition %in% amr_cond & cpeb_travel_os_in_past_4y=="Yes" & 
                                      cpeb_travel_os_in_past_4y_medical_treatment_intended=="Yes" & 
    ...
    rmk_facility_oth = if_else(condition %in% amr_cond & cpeb_travel_os_in_past_4y=="Yes" & cpeb_travel_os_in_past_other=="Yes" & 
    ...
    rmk_healthcare_type = if_else(condition %in% amr_cond & cpeb_travel_os_in_past_4y=="Yes" & 
                                    cpeb_travel_os_in_past_4y_medical_treatment=="Yes" & 
                                    is.na(cpeb_travel_os_in_past_4y_medical_treatment_type), 
                                  "Specify type of treatment received missing", NA_character_),
    # this field (cpeb_travel_os_in_past_4y_illness) doesn't seem to exist anymore
...
amr_keymiss_table <- set_keymiss_table((amr_key_missing_rmk))
output_keymiss_table(amr_keymiss_table)
...
oth_inprogress_table <- set_inprogress_table("Other Conditions", 4)
output_inprogress_table("Other Conditions", oth_inprogress_table)
...
outbreak_new <- ob_data1mth %>% 
  ...
  mutate(url = paste0('https://phess.dhhs.vic.gov.au/main.do?CaseID=', phess_id)) %>%
  select(phess_id, url, event_date, event_name, condition, investigation_status, disease_type)
...
legion_open <- legion_ob %>% 
  ...
  mutate(remarks = "Outbreak status is still open after 90 days") %>% 
  select(phess_id, url, event_date, organism, investigation_status, event_name, remarks, disease_type)
...
if(nrow(legion_open)==0) {
    ...
    flextable(legion_open, col_keys=c("phess_id", "organism", "event_date", "investigation_status", "event_name")) %>% 
      compose(j="phess_id", value=as_paragraph(hyperlink_text(x=phess_id, url=url))) %>% 
      color(j="phess_id", color="#0077CC") %>% 
...
resp_ob_data_rmk <- resp_ob_data1mth %>%  
  ...
  mutate(url = paste0('https://phess.dhhs.vic.gov.au/main.do?CaseID=', phess_id),
         ...
         rmk_incident = if_else(incident_classification == "Not yet classified", "Assign a category classification for this incident", NA_character_), 
...
resp_ob_keymiss_table <- set_keymiss_ob_table(resp_ob_data_rmk)
output_keymiss_ob_table(resp_ob_keymiss_table)
...
gastro_ob_keymiss_table <- set_keymiss_ob_table(gastro_ob_data_rmk)
output_keymiss_ob_table(gastro_ob_keymiss_table)

```


```{r output_excel}
tab_order <- c("Resp Outbreaks", "FBWB Outbreaks", "BBV", "STI", "Enteric", "Vector-borne", "Zoonotic", 
               "AMR", "Other", "Vaccine Preventable")

excel_table <- bind_rows(outbreak_new, legion_open, resp_ob_keymiss_table, gastro_ob_keymiss_table, 
                         bbv_keymiss_table, bbv_inprogress_table, ent_keymiss_table, ent_inprogress_table, crypto_risk_table, 
                         sti_keymiss_table, sti_inprogress_table, mpox_keymiss_table, 
                         vpd_keymiss_table, vpd_inprogress_table, vbd_keymiss_table, vbd_inprogress_table, 
                         amr_keymiss_table, oth_inprogress_table) %>% 
  select(disease_type, phess_id, condition, remarks, date_identified, event_name) %>% 
  mutate(tab = case_when(disease_type=="Vaccine Preventable Diseases" & !is.na(event_name) ~ "Resp Outbreaks", 
                         disease_type=="Enteric Diseases" & !is.na(event_name) ~ "FBWB Outbreaks", 
                         disease_type=="Blood Borne Viruses" & is.na(event_name) ~ "BBV", 
                         disease_type=="Sexually Transmissible Infections" & is.na(event_name) ~ "STI", 
                         condition=="Mpox" & is.na(event_name) ~ "STI", 
                         disease_type=="Enteric Diseases" & is.na(event_name) ~ "Enteric", 
                         disease_type=="Vector Borne Diseases" & is.na(event_name) ~ "Vector-borne", 
                         disease_type=="Zoonotic" & is.na(event_name) ~ "Zoonotic", 
                         condition %in% amr_cond & is.na(event_name) ~ "AMR", 
                         disease_type=="Other Conditions" & is.na(event_name) ~ "Other", 
                         disease_type=="Vaccine Preventable Diseases" & is.na(event_name) ~ "Vaccine Preventable"
                         )) %>% 
  select(-disease_type, -event_name) %>% 
  relocate(tab, .before = phess_id) %>% 
  arrange(factor(tab, levels=tab_order)) %>% 
  rename(Tab = tab, 
         `PHESS ID` = phess_id, 
         Condition = condition, 
         `Issue identified` = remarks, 
         `Date identified` = date_identified)

writexl::write_xlsx(excel_table, here("Output", paste0("DataQualChk", "-", format(Sys.Date(), "%d %b %Y"), ".xlsx")))
```
