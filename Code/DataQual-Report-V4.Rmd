...
# all.raw <- readxl::read_xlsx(file.path(phess_data_dir, condition_all_file), sheet=1, guess_max=min(500000, Inf)) %>% janitor::clean_names() 
...
cond_data_mth7 <- filter(condition.clean, event_date <= as.Date(start_date7mth)) %>% 
  filter(event_type=="Case") # incl cases where event data is before the 7mth cutoff (for MU condition)
